...
then
    if [ ! -f "$B/master.cfg" ]
    then
        err "No master.cfg found nor \$BUILDBOT_CONFIG_URL!"
        err "Please provide a master.cfg file in $B or provide a \$BUILDBOT_CONFIG_URL variable via -e"
        exit 1
    fi
    ...
    then
        until fetch -qo - "$BUILDBOT_CONFIG_URL" | tar -xz --strip-components=1 --directory="$B/$BUILDBOT_CONFIG_DIR"
        do
            warn "Can't download from \$BUILDBOT_CONFIG_URL: $BUILDBOT_CONFIG_URL"
            sleep 1
        done
        ...
        until fetch -qo - "$BUILDBOT_CONFIG_URL" > "$B/master.cfg"
        do
            warn "Can't download from $$BUILDBOT_CONFIG_URL: $BUILDBOT_CONFIG_URL"
        done
    fi
...
do
    warn "Can't upgrade master yet. Waiting for database ready?"
    sleep 1
done
